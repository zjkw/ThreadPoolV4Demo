...
ThreadErrorCode PostMsg(const task_name_t& target_name, const task_cmd_t& cmd, task_data_t data, const task_flag_t& flags/* = task_flag_null*/, const task_echofunc_t& echofunc/* = nullptr*/)//target_id为0表示广播
{
	//参数检查
...
//任务管理
ThreadErrorCode AddManagedTask(const task_cls_t& cls, const task_name_t& name, const task_param_t& param, const task_routinefunc_t& routine, task_id_t& id)	//调节任务
{
	return 	tixAddManagedTask(cls, name, param, routine, id);
}
ThreadErrorCode DelManagedTask(const task_id_t& id)//是否等待目标关闭，需要明确的是如果自己关闭自己或关闭非托管线程，将会是强制改成异步
{
	//参数检查
